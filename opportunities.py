...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r2-preview"
...
def _load_b32_jp_additions():
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "v0.04-r4.2-b32-jp-batch01-2":
...
def validate_curated_opportunities():
    ...
    expected_non_tw_spots = {"jp-003", "jp-005"}
    ...
    if len(opportunity_ids) != 192:
        errors.append(f"expected 192 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 202:
        errors.append(f"expected 202 variants, got {len(variant_ids)}")
    if viewpoint_relations != 197:
        errors.append(f"expected 197 profile_viewpoint relations, got {viewpoint_relations}")
