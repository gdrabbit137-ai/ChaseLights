...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r11-preview"
...
def _load_b32_jp_additions():
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "v0.04-r4.2-b32-jp-batch01-11":
...
def validate_curated_opportunities():
    ...
    expected_non_tw_spots = {"jp-001", "jp-002", "jp-003", "jp-004", "jp-005", "jp-006", "jp-007", "jp-008", "jp-009", "jp-010", "jp-013", "jp-019"}
    ...
    if len(opportunity_ids) != 203:
        errors.append(f"expected 203 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 213:
        errors.append(f"expected 213 variants, got {len(variant_ids)}")
    if viewpoint_relations != 208:
        errors.append(f"expected 208 profile_viewpoint relations, got {viewpoint_relations}")
