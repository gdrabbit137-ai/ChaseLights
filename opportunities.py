...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r23-preview"
...
def validate_curated_opportunities():
    ...
    if len(opportunity_ids) != 220:
        errors.append(f"expected 220 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 230:
        errors.append(f"expected 230 variants, got {len(variant_ids)}")
    if viewpoint_relations != 225:
        errors.append(f"expected 225 profile_viewpoint relations, got {viewpoint_relations}")
