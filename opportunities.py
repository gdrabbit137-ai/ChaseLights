...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r24-preview"
...
def _load_b32_jp_additions():
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "v0.04-r4.2-b32-jp-batch01-24":
...
for _region_key in ("tw", "jp", "us"):
    _region_spots = [
        spot for spot in _ACTIVE_SPOTS
        if str(spot.get("spot_id") or "").startswith(f"{_region_key}-")
    ]
    REGION_CATALOG_COUNTS[_region_key] = {
        "spots": len(_region_spots),
        "opportunities": sum(len(spot.get("opportunities", [])) for spot in _region_spots),
        "condition_variants": sum(
            len(opportunity.get("condition_variants", []))
            for spot in _region_spots
            for opportunity in spot.get("opportunities", [])
        ),
        "profile_viewpoint_relations": sum(
            len(opportunity.get("viewpoints", []))
...
def get_opportunities(region_key, spot_id):
    ...
    region_key = str(region_key or "").lower()
    if not str(spot_id or "").startswith(f"{region_key}-"):
        return []
...
def validate_curated_opportunities():
    ...
    expected_non_tw_spots = {"jp-001", "jp-002", "jp-003", "jp-004", "jp-005", "jp-006", "jp-007", "jp-008", "jp-009", "jp-010", "jp-011", "jp-012", "jp-013", "jp-014", "jp-015", "jp-016", "jp-017", "jp-018", "jp-019", "jp-020", "jp-021", "jp-022", "jp-023", "jp-024", "jp-025"}
    ...
    for spot_id, opportunities in CURATED_OPPORTUNITIES.items():
        if not spot_id.startswith(("tw-", "jp-", "us-")):
            errors.append(f"{spot_id}: unsupported region catalog key")
    ...
    if len(opportunity_ids) != 222:
        errors.append(f"expected 222 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 232:
        errors.append(f"expected 232 variants, got {len(variant_ids)}")
    if viewpoint_relations != 227:
        errors.append(f"expected 227 profile_viewpoint relations, got {viewpoint_relations}")
