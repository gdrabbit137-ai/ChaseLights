...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r7-preview"
...
def _load_b32_jp_additions():
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "v0.04-r4.2-b32-jp-batch01-7":
...
def validate_curated_opportunities():
    ...
    expected_non_tw_spots = {"jp-001", "jp-002", "jp-003", "jp-004", "jp-005", "jp-009", "jp-010"}
    ...
    if len(opportunity_ids) != 198:
        errors.append(f"expected 198 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 208:
        errors.append(f"expected 208 variants, got {len(variant_ids)}")
    if viewpoint_relations != 203:
        errors.append(f"expected 203 profile_viewpoint relations, got {viewpoint_relations}")
