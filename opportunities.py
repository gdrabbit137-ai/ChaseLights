...
ADAPTER_VERSION = "v0.04-r4.2-b32-jp-batch01-r4-preview"
...
def _load_b32_jp_additions():
    ...
    payload = json.loads(path.read_text(encoding="utf-8"))
    if payload.get("schema_version") != "v0.04-r4.2-b32-jp-batch01-4":
...
def validate_curated_opportunities():
    ...
    expected_non_tw_spots = {"jp-001", "jp-003", "jp-004", "jp-005"}
    ...
    if len(opportunity_ids) != 194:
        errors.append(f"expected 194 opportunities, got {len(opportunity_ids)}")
    if len(variant_ids) != 204:
        errors.append(f"expected 204 variants, got {len(variant_ids)}")
    if viewpoint_relations != 199:
        errors.append(f"expected 199 profile_viewpoint relations, got {viewpoint_relations}")
